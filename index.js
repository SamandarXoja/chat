...
io.on('connection', (socket) => {
    ...
    socket.on('newChatMessage', async (messageData) => {
        try {
            ...
            socket.broadcast.emit('chatMessage', newChatMessage);

            const chats = await Chat.find();
            if (!arraysEqual(chats, lastChatsList)) {
                lastChatsList = chats;
                io.emit('chatsList', chats)
            }
...
function arraysEqual(arr1, arr2) {
    ...
    for (let i = 0; i < arr1.length; i++) {
        if (!arr2.find(item => item._id.toString() === arr1[i]._id.toString())) {
            return false;
        }
    }
    return true;
